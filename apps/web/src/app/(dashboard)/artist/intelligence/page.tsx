...
function trendArrow(trend: 'up' | 'down' | 'stable') {
  ...
  if (trend === 'stable') return <span className="text-nocturne-text-secondary ml-1">—</span>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="glass-card bg-gradient-to-br from-purple-500/10 to-transparent p-5 border-purple-400/30 hover-glow">
          <div className="flex items-start justify-between mb-2">
            <p className="text-xs font-display font-semibold text-nocturne-text-secondary uppercase tracking-wider">Trust Score</p>
          ...
          <p className="text-xs text-nocturne-text-secondary mt-2">Community rating</p>
        </div>

        {/* Booking Velocity */}
        <div className="glass-card bg-gradient-to-br from-primary-500/10 to-transparent p-5 border-primary-400/30 hover-glow">
          <div className="flex items-start justify-between mb-2">
            <p className="text-xs font-display font-semibold text-nocturne-text-secondary uppercase tracking-wider">Booking Velocity</p>
            <Zap size={16} className="text-nocturne-accent opacity-60" />
          </div>
          <p className="text-2xl font-display font-bold text-nocturne-accent">{data.booking_velocity_30d ?? 0}</p>
          <p className="text-xs text-nocturne-text-secondary mt-2">30 days / {data.booking_velocity_90d ?? 0} over 90d</p>
        </div>

        {/* Rebook Rate */}
        <div className="glass-card bg-gradient-to-br from-green-500/10 to-transparent p-5 border-green-400/30 hover-glow">
          <div className="flex items-start justify-between mb-2">
            <p className="text-xs font-display font-semibold text-nocturne-text-secondary uppercase tracking-wider">Rebook Rate</p>
            <BarChart3 size={16} className="text-green-400 opacity-60" />
          </div>
          <p className="text-2xl font-display font-bold text-green-300">{data.rebook_rate ?? 0}%</p>
          <p className="text-xs text-nocturne-text-secondary mt-2">Client loyalty</p>
        </div>

        {/* Demand Alignment */}
        <div className="glass-card bg-gradient-to-br from-blue-500/10 to-transparent p-5 border-blue-400/30 hover-glow">
          <div className="flex items-start justify-between mb-2">
            <p className="text-xs font-display font-semibold text-nocturne-text-secondary uppercase tracking-wider">Demand Alignment</p>
            <Target size={16} className="text-blue-400 opacity-60" />
          </div>
          <p className="text-2xl font-display font-bold text-blue-300">{data.demand_alignment ?? 0}%</p>
          <p className="text-xs text-nocturne-text-secondary mt-2">Market fit</p>
        </div>

        {/* Total Bookings */}
        <div className="glass-card bg-gradient-to-br from-pink-500/10 to-transparent p-5 border-pink-400/30 hover-glow">
          <div className="flex items-start justify-between mb-2">
            <p className="text-xs font-display font-semibold text-nocturne-text-secondary uppercase tracking-wider">Total Bookings</p>
            <BarChart3 size={16} className="text-pink-400 opacity-60" />
          </div>
          <p className="text-2xl font-display font-bold text-pink-300">{data.total_bookings_this_year ?? 0}</p>
          <p className="text-xs text-nocturne-text-secondary mt-2">This year</p>
        </div>

        {/* Crowd Energy */}
        <div className="glass-card bg-gradient-to-br from-orange-500/10 to-transparent p-5 border-orange-400/30 hover-glow">
          <div className="flex items-start justify-between mb-2">
            <p className="text-xs font-display font-semibold text-nocturne-text-secondary uppercase tracking-wider">Crowd Energy</p>
            <Zap size={16} className="text-orange-400 opacity-60" />
          </div>
          <p className="text-2xl font-display font-bold text-orange-300">{data.avg_crowd_energy ?? 0}/5</p>
          <p className="text-xs text-nocturne-text-secondary mt-2">Audience feedback</p>
        </div>
      </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          <h2 className="text-lg font-display font-semibold text-nocturne-text-primary">Top Cities</h2>
          <MapPin size={20} className="text-nocturne-accent opacity-60" />
        </div>
        <div className="flex flex-wrap gap-2">
          {(data.top_cities ?? []).length === 0 ? (
            <div className="glass-card w-full p-6 text-center">
              <p className="text-nocturne-text-secondary text-sm">No city data yet. Keep performing to build your city presence!</p>
            </div>
          ) : (
            data.top_cities.map((c) => (
              ...
              >
                ...
                {c.city}
                <span className="bg-gradient-nocturne/20 border border-nocturne-accent/30 text-nocturne-accent text-xs px-2 py-0.5 rounded-full font-bold">
                  {c.count}
                </span>
              </span>
            ))
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          <h2 className="text-lg font-display font-semibold text-nocturne-text-primary">Top Event Types</h2>
          <Target size={20} className="text-nocturne-accent opacity-60" />
        </div>
        <div className="flex flex-wrap gap-2">
          {(data.top_event_types ?? []).length === 0 ? (
            <div className="glass-card w-full p-6 text-center">
              <p className="text-nocturne-text-secondary text-sm">No event type data yet. Your specialties will appear here.</p>
            </div>
          ) : (
            data.top_event_types.map((e) => (
              ...
              >
                ...
                {e.event_type}
                <span className="bg-gradient-nocturne/20 border border-secondary-400/30 text-secondary-300 text-xs px-2 py-0.5 rounded-full font-bold">
                  {e.count}
                </span>
              </span>
            ))
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Zap size={18} className="text-nocturne-accent opacity-60 group-hover:opacity-100 transition-opacity" />
              <h3 className="font-display font-semibold text-nocturne-text-primary">Gig Advisor</h3>
            </div>
            <p className="text-xs text-nocturne-text-secondary">Smart recommendations</p>
          </div>
          <ArrowRight size={14} className="text-nocturne-accent opacity-0 group-hover:opacity-100 transition-opacity mt-3" />
        </Link>
        ...
        >
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-semibold text-nocturne-text-primary">Reputation</h3>
            </div>
            <p className="text-xs text-nocturne-text-secondary">Reviews & insights</p>
          </div>
        ...
        >
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-semibold text-nocturne-text-primary">Seasonal</h3>
            </div>
            <p className="text-xs text-nocturne-text-secondary">Trends & timing</p>
          </div>
        ...
        >
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display font-semibold text-nocturne-text-primary">Financial</h3>
            </div>
            <p className="text-xs text-nocturne-text-secondary">Cash flow & taxes</p>
          </div>
