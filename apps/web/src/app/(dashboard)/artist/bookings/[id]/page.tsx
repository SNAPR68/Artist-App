...
export default function ArtistBookingDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in-up">
      <div>
        <h1 className="text-3xl font-bold text-gradient font-heading">Booking from {booking.client_name ?? 'Client'}</h1>
        <p className="text-text-muted text-sm mt-1">Review event details and submit your quote</p>
      </div>
      ...
      <div className="glass-card rounded-xl p-6 space-y-4 backdrop-blur-xl border glass-border">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {booking.guest_count && <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-text-muted text-xs uppercase tracking-wide">Guests</p>
              <p className="text-text-primary font-medium">{booking.guest_count}</p>
            </div>
          </div>}
      ...
      {canQuote && autoQuote && booking.quotes.length === 0 && (
        <div className="glass-card rounded-xl p-6 backdrop-blur-xl border-2 border-gradient-accent bg-gradient-to-r from-blue-500/10 to-purple-500/10 shadow-glow-sm hover:shadow-lg transition-all">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-text-muted mt-0.5">Based on your pricing settings</p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full px-4 py-2.5 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-lg text-sm font-semibold hover-glow shadow-glow-sm transition-all"
          >
            Use this Amount
          </button>
        </div>
      ...
      {booking.quotes.length > 0 && (
        <div className="glass-card rounded-xl p-6 space-y-4 backdrop-blur-xl border glass-border">
          <h2 className="text-sm font-semibold text-gradient uppercase tracking-wide font-heading">Negotiation History</h2>
          {booking.quotes.map((q, idx) => (
            <div key={q.round} className={`glass-medium rounded-lg p-4 border glass-border backdrop-blur-md ${
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  <span className="text-sm font-semibold text-text-primary">Round {q.round} {q.is_final ? '• Final Offer' : ''}</span>
      ...
      {canQuote && (
        <div className="glass-card rounded-xl p-6 space-y-3 backdrop-blur-xl border glass-border">
          ...
          <form onSubmit={handleSubmitQuote} className="space-y-3">
            <div className="flex gap-2">
              ...
              <button type="submit" disabled={submitting} className="px-4 py-2.5 bg-gradient-to-r from-primary-500 to-accent-magenta text-white rounded-lg text-sm font-semibold hover-glow shadow-glow-sm disabled:opacity-50 transition-all flex items-center gap-2 whitespace-nowrap">
      ...
      <div className="flex gap-3 flex-col sm:flex-row">
        {canConfirm && (
          <button onClick={() => handleTransition('confirmed')} disabled={submitting} className="flex-1 py-2.5 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg text-sm font-semibold hover-glow shadow-glow-sm disabled:opacity-50 transition-all flex items-center justify-center gap-2">
